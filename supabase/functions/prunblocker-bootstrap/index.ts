 ...
 * the exact same pattern as onboardtime-bootstrap. Modules never share
 * org-scoped RLS across schemas, so each module bootstraps through its OWN
 * edge function (public.orgs/org_members are shared, but the module edge fn
 * is the module's entry point).
 *
 * WHY service role: same rationale as onboardtime — plain table inserts
 * with the SERVICE_ROLE_KEY secret (server-side only) sidestep the stuck
 * PostgREST function-catalog bug and always work once the secret is set.
 */
